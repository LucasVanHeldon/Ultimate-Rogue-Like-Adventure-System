...
///////////////////////////////////////////
//iEyeType: Integer
//Function: Gets the racial type and gender and returns
//          eye type for visual effect
//oPC: Creature to run the check on
//iCol: int 1 = red, 2 = green
//
int iEyeType(object oPC, int iCol);
...
{
 int iEyes;
 int iRCheck = GetRacialType(oPC);
 int iSCheck = GetGender(oPC);

 if (iCol==1)
{
if (iRCheck == RACIAL_TYPE_HUMAN)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_HUMAN_MALE;
     else iEyes = VFX_EYES_RED_FLAME_HUMAN_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_DWARF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_DWARF_MALE;
     else iEyes = VFX_EYES_RED_FLAME_DWARF_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_GNOME)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_GNOME_MALE;
     else iEyes = VFX_EYES_RED_FLAME_GNOME_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFELF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_HALFELF_MALE;
     else iEyes = VFX_EYES_RED_FLAME_HALFELF_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFLING)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_HALFLING_MALE;
     else iEyes = VFX_EYES_RED_FLAME_HALFLING_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFORC)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_HALFORC_MALE;
     else iEyes = VFX_EYES_RED_FLAME_HALFORC_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_ELF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_RED_FLAME_ELF_MALE;
     else iEyes = VFX_EYES_RED_FLAME_ELF_FEMALE;
    }
}
else if (iCol==2)
{
if (iRCheck == RACIAL_TYPE_HUMAN)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_HUMAN_MALE;
     else iEyes = VFX_EYES_GREEN_HUMAN_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_DWARF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_DWARF_MALE;
     else iEyes = VFX_EYES_GREEN_DWARF_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_GNOME)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_GNOME_MALE;
     else iEyes = VFX_EYES_GREEN_GNOME_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFELF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_HALFELF_MALE;
     else iEyes = VFX_EYES_GREEN_HALFELF_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFLING)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_HALFLING_MALE;
     else iEyes = VFX_EYES_GREEN_HALFLING_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_HALFORC)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_HALFORC_MALE;
     else iEyes = VFX_EYES_GREEN_HALFORC_FEMALE;
    }
if (iRCheck == RACIAL_TYPE_ELF)
    {
     if (iSCheck == GENDER_MALE) iEyes = VFX_EYES_GREEN_ELF_MALE;
