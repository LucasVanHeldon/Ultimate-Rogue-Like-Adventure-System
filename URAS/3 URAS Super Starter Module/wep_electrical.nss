...
void main()
{
    ...
    itemproperty iPF = ItemPropertyDamageBonus(IP_CONST_DAMAGETYPE_ELECTRICAL,IP_CONST_DAMAGEBONUS_1d4);
    itemproperty iVis = ItemPropertyVisualEffect(ITEM_VISUAL_ELECTRICAL);
...
// * is activated. Note that this event fires for PCs only
            ...
            {
                SendMessageToPC(GetFirstPC(),"Active");
                IPSafeAddItemProperty(oItem,iPF,RoundsToSeconds(time),X2_IP_ADDPROP_POLICY_KEEP_EXISTING);
                IPSafeAddItemProperty(oItem,iVis,RoundsToSeconds(time),X2_IP_ADDPROP_POLICY_KEEP_EXISTING);
            }
