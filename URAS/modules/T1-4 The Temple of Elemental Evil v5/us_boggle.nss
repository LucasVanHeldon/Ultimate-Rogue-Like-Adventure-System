...
void main()
{
   ...
   //SetSpawnInCondition(NW_FLAG_PERCIEVE_EVENT);
    ...
    SetLocalLocation(OBJECT_SELF,"oStartPos",GetLocation(OBJECT_SELF));
    ...
    for(i = 0; i < d4(); i++) CreateStdPotion();
    for(i = 0; i < d4(); i++) CreateStdScroll();
    ...
    //for(i = 0; i < 3+d4(); i++) CreateObject(OBJECT_TYPE_CREATURE,"nw_goblina",GetLocation(OBJECT_SELF));
    //for(i = 0; i < d3(); i++) CreateObject(OBJECT_TYPE_CREATURE,"nw_spidgiant",GetLocation(OBJECT_SELF));
}
