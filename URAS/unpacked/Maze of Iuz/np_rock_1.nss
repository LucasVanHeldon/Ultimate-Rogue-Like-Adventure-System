...
void main()
{
    ...
    object oDWP = GetWaypointByTag("WP_TEL_ROCK");
    // get the faceing of the Waypoint and set it on the object.
    float fFace = GetFacing(oDWP);
    AssignCommand (oDWP, SetFacing(fFace) );
    ...
    if(!GetIsPC(oPC))
        return;
    //Ok, if the PC has been in the teleport before, let's give them a 1
    if(!(GetLocalInt(oPC, "nTrapActivated")==1))
    ...
    if (iTeleportWholeParty == 1)
        ...
        while(GetIsObjectValid(oFM))
            ...
            SetLocalInt(oPC, "nTrapActivated", 1);
