...
int X2_UMD()
{
    ...
    // alow classes with UseMagicDevice skill to use items of their class still
    itemproperty ip = GetFirstItemProperty(oItem);
    while(GetIsItemPropertyValid(ip))
    {
        if(GetItemPropertyType(ip) == ITEM_PROPERTY_USE_LIMITATION_CLASS)
        {
            int nClass1 = GetClassByPosition(0,oCaster);
            int nClass2 = GetClassByPosition(1,oCaster);
            int nClass3 = GetClassByPosition(2,oCaster);
            int p1 = GetItemPropertyParam1(ip);
            int p2 = GetItemPropertyParam1Value(ip);
            if(p1 == nClass1 || p1 == nClass2 || p1 == nClass3) return TRUE;
            if(p2 == nClass1 || p2 == nClass2 || p2 == nClass3) return TRUE;
        }
