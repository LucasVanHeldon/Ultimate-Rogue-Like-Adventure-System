...
{
    int nSummons = GetLocalInt(OBJECT_SELF,"bSummons");

    if( d6() < 4 &&  nSummons < 3 )
    {
        SetLocalInt(OBJECT_SELF,"bSummons",nSummons+1);
        int x = Random(2);
        int n = d4(4)+1;
        int i;
        switch(x)
...
void SpecialAttack()
{
    switch(d2())
    {
    case 1: DEMONIC_SpecialAttack();
    case 2: VAMPIRE_SpecialAttack();
    }
}
...
void main()
{
    ...
    case 1011:
        ...
        if( d6() < 4 && GetIsEnemy(oSC) )
        {
            DEMONIC_SpellCast(oSC);
        }
        break;
    }
}
