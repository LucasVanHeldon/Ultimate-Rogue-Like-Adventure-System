void _RunOnce(object o)
{
    int isOk = FALSE;
    object oArea = GetFirstArea();
    while(GetIsObjectValid(oArea))
    {
        string s = GetName(oArea);
        if(GetLocalInt(oArea,"bRunOnce") == 3)
        {
            oArea = GetNextArea();
            continue;
        }
        SetLocalInt(oArea,"bRanOnce",TRUE);
        ...
        if(isOk)
        {
            object o = CreateObject(OBJECT_TYPE_PLACEABLE,"randomencounters",GetLocation(o));
            if(GetIsObjectValid(o))
                SendMessageToPC(GetFirstPC(),"Random Encounters");
        }
        oArea = GetNextArea();
    }
    SetLocalInt(GetArea(o),"bRunOnce",3);

}

void OC_RunOnce()
{
    if(GetLocalInt(GetArea(OBJECT_SELF),"bRunOnce")!= 3)
            DelayCommand(0.1,_RunOnce(OBJECT_SELF));
