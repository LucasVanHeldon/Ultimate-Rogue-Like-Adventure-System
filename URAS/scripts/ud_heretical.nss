// Heretical Template (undead+demonic+death magic)
// All Demons/Devils can control undead, except for aberrant/eldritch/chaosborn
// By Weed Wizard
...
void HERETICAL_SpecialAttack(object oTarget = OBJECT_INVALID)
{
    ...
    if(GetLocalInt(OBJECT_SELF,"bCircleOfDeath"))
        if(d6() < 3)
        {
            MMP_CircleOfDeath(oTarget);
            return;
        }
    if(GetLocalInt(OBJECT_SELF,"bCircleOfDoom"))
        if(d6() < 3)
        {
            MMP_CircleOfDoom(oTarget);
            return;
        }
    if(GetLocalInt(OBJECT_SELF,"bDeathArmor"))
        if(d6() < 3)
        {
            MMP_DeathArmor(oTarget);
            return;
        }
    if(GetLocalInt(OBJECT_SELF,"bDeathWard"))
        if(d6() < 3)
        {
            MMP_DeathWard(oTarget);
            return;
        }
    if(GetLocalInt(OBJECT_SELF,"bFingerOfDeath"))
        if(d6() < 3)
        {
            MMP_FingerOfDeath(oTarget);
            return;
        }
    if(GetLocalInt(OBJECT_SELF,"bDirge"))
        if(d6() < 3)
        {
            MMP_Dirge(oTarget);
            return;
