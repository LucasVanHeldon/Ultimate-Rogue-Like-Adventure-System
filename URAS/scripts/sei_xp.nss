...
// PW fix by Catscan
// sr 5.5
// pw exp trainer fix by Catscan
// SR5.3
// changed xp reward to reward minimum 0 experience instead of 1.
...
// BEGIN ADDED BY CATSCAN:  Function prototype
// Returns the amount of experience that would be rewarded to the character
//  ARGUMENTS:
//      a_oCharacter    = The character to check
//      a_nXP           = The base experience
int XP_GetXPReward(object a_oCharacter, int a_nXP);
// END ADDED BY CATSCAN
...
// BEGIN ADDED BY CATSCAN
// I chose delegation to XP_RewardXP to make sure the same
// code path was used that XP_RewardXP used when
// giving XP.  This change seemed the easiest and didn't
// end up duplicating a lot of code.  Just use the existing.
// Return the XP to be rewarded to the character
//
int XP_GetXPReward( object a_oCharacter, int a_nXP )
{
    // This will be checked later in dnd_level
    SetLocalInt( a_oCharacter, "DONOTREWARD", TRUE );

    // Delegate call to XP_RewardXP to determine total
    // XP after adjuments have been made
    XP_RewardXP( a_oCharacter, a_nXP );
    int retXP = GetLocalInt( a_oCharacter, "XPREWARD" );
    DeleteLocalInt( a_oCharacter, "XPREWARD" );
    return retXP;
} // END XP_GetXPReward
// END ADDED BY CATSCAN
...
//////////////////////////////
// My change
// Make certain that zero experience is awarded when appropriate.
    ...
    {
        // BEGIN MODIFIED BY CATSCAN -- This prevents XP from being
        // rewarded twice when PWEXP = 0.
        if (GetLocalInt( a_oCharacter, "DONOTREWARD"))
        {
            DeleteLocalInt( a_oCharacter, "DONOTREWARD");
            SetLocalInt( a_oCharacter, "XPREWARD", nXP);
        }
        // END MODIFIED BY CATSCAN
        else
        {
