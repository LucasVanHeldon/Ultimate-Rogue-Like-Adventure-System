...
#include "NW_I0_GENERIC"
 ...
 void StickItUpHere(object Target){
    if(d20() < 3){
    effect Drain = EffectNegativeLevel(2);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT, Drain, Target);

    effect Test = EffectVisualEffect(VFX_COM_HIT_NEGATIVE);
        ApplyEffectToObject(DURATION_TYPE_INSTANT, Test, Target);
        }
    if(d20() < 3){

    effect Disease = EffectDisease(DISEASE_MINDFIRE);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT, Disease, Target);
    effect VisDisease = EffectVisualEffect(VFX_IMP_DISEASE_S);
        ApplyEffectToObject(DURATION_TYPE_INSTANT, VisDisease, Target);



    }
    if(d100() < 3){
        if(GetHitDice(Target) < 6 || GetCharacterLevel(Target) < 6){
    effect Smackdown = EffectDeath();
    ApplyEffectToObject(DURATION_TYPE_INSTANT, Smackdown, Target);
    effect VisSmackdown = EffectVisualEffect(VFX_COM_CHUNK_RED_MEDIUM);
        ApplyEffectToObject(DURATION_TYPE_INSTANT,  VisSmackdown, Target);
        }
    if(GetHitDice(Target) >= 6 || GetCharacterLevel(Target) >= 6){
    effect Smackdown = EffectDamage(10, DAMAGE_TYPE_PIERCING);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, Smackdown, Target);
    effect VisSmackdown = EffectVisualEffect(VFX_COM_BLOOD_REG_RED);
        ApplyEffectToObject(DURATION_TYPE_INSTANT,  VisSmackdown, Target);
        }
    }
    if(d20() == 1){
    ActionCastSpellAtObject(SPELL_ENERVATION, Target, METAMAGIC_ANY, TRUE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
        }
    }
