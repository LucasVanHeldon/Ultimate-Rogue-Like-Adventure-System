...
int MMPABB_SpecialAttack(object oTarget = OBJECT_INVALID)
{
...
int MMPMRBD_SpecialAttack(object oTarget = OBJECT_INVALID)
{
...
int OOZE_SpecialAttack(object oTarget = OBJECT_INVALID)
{
    int nHD = GetHitDice(OBJECT_SELF);
...
void main()
{
    ...
    if(d6() < 3)
    {
        ...
        if( MMPABB_SpecialAttack(oEnemy) )
        {
             __TurnCombatRoundOn(FALSE);
        }
        else if(MMPMRBD_SpecialAttack(oEnemy) )
        {
             __TurnCombatRoundOn(FALSE);
        }
        else if(OOZE_SpecialAttack(oEnemy) )
        {
             __TurnCombatRoundOn(FALSE);
        }
        else if(GetLocalInt(OBJECT_SELF,"nDemonic")==1)
        {
