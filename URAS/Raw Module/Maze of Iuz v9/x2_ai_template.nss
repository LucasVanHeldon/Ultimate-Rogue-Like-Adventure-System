// This is for the template compiler.
// You shouldn't give them randomly to creatures unless you really want to, which I don't recommend.
...
int Template_AbilityBuff(object oTarget= OBJECT_SELF);
int Template_AbilitySummon(object oTarget = OBJECT_SELF);
int Template_AbilityOffensive(object oTarget);

int Template_CastBuff(object oTarget = OBJECT_SELF);
int Template_CastSneak(object oTarget);
int Template_CastRemoveEffect(object oTarget);
int Template_CastProtection(object oTarget);
int Template_CastHealing(object oTarget);
int Template_CastRestoration(object oTarget);
int Template_CastPsychology(object oTarget);
int Template_DebuffSpellCaster(object oTarget);
int Template_CastAnimateDead(object oTarget);
int Template_CastSummon(object oTarget);
int Template_CastPlanar(object oTarget);
int Template_CastDispelMagic(object oTarget);
int Template_CastOffensive(object oTarget);
...
int Template_CastBuff(object oTarget = OBJECT_SELF)
{
//    SendMessageToPC(GetFirstPC(),"Cast Buff");
...
/* The talent engine will use this, it doesn't work very well anyways for hostile AI, but the AI will ignore PC usually.
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Spells which help in sneaking/hiding etc
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// only cast on allies... unless you want to raise enemies to kill them again???
int Template_CastRaiseDead(object oTarget)
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Psychological and Hindering spells.
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/* don't do epic spells, let the talent AI do it.
...
/* sort of useless without companion
...
{
    object oEnemy;
    float D = GetDistanceToObject(FindNearestEnemy(OBJECT_SELF));

    int nothing = 0;

    switch(d3())
    {
    case 1: oEnemy = FindWeakestEnemy(OBJECT_SELF); break;
    case 2: oEnemy = FindStrongestEnemy(OBJECT_SELF); break;
    case 3: oEnemy = FindNearestEnemy(OBJECT_SELF); break;
    }
    ...
    {
        nothing = 1;
        if(!Template_AbilityOffensive(oEnemy))
                if(!Template_CastPsychology(oEnemy))
                    nothing = 0;
    }
    if(nothing == 0 && D > 10.0)
    {
        nothing = 1;
            if(!Template_AbilityBuff())
                    if(!Template_AbilitySummon(OBJECT_SELF))
                        if(!Template_CastBuff(OBJECT_SELF))
                            if(!Template_CastProtection(OBJECT_SELF))
                                    if(!Template_CastSummon(oEnemy))
                                        nothing = 0;
    }
    ...
    if(nothing == 0)
    {
        ...
        for( i = 0; i < n; i++)
        {
            switch(Random(8))
            {
            case 0: if(nothing=Template_AbilityOffensive(oEnemy)) break;
            case 1: if(nothing=Template_AbilitySummon()) break;
            case 2: if(nothing=Template_AbilityBuff()) break;
            case 3: if(nothing=Template_CastBuff(OBJECT_SELF)) break;
            case 4: if(nothing=Template_CastProtection(OBJECT_SELF)) break;
            case 5: if(nothing=Template_CastPsychology(oEnemy)) break;
            case 6: if(nothing=Template_CastDebuff(oEnemy)) break;
            case 7: if(nothing=Template_CastSummon(oEnemy)) break;
            case 8: if(nothing=Template_CastOffensive(oEnemy)) break;
            }
            if(nothing == 1) break;
        }
    }
    //SendMessageToPC(GetFirstPC(),"Nothing="+IntToString(nothing));
