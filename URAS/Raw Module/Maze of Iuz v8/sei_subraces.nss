...
// Lorinton change
// Use a constant to indicate a mix of subrace hide and effects.
int USE_SUBRACE_MIX         = TRUE;
...
////////////////////////////
// Lorinton change
// Don't bring up the dialog. If the subrace field is wrong just assume there is no subrace.
/*
            if( GetIsPC( a_oCharacter ) )
            {
...
void SEI_RemoveEffect( object a_oCharacter, effect a_eEffect )
{
    ...
    if(( GetEffectCreator( a_eEffect ) != GetModule() ) &&
        ( GetEffectSubType( a_eEffect ) != SUBTYPE_EXTRAORDINARY ) )  // Lorinton change, was supernatural
    {
        RemoveEffect( a_oCharacter, a_eEffect );
        // Lorinton change, the RemoveEffect function appears to be very buggy, redo the subrace afterwards
...
void SEI_RemoveEffects( object a_oCharacter )
{
    ...
    {
        // Lorinton change
        // Added the check for extraordinary effect
        if(( GetEffectCreator( a_eEffect ) != GetModule() ) &&
                               ...
                               int a_iEffectType = EFFECT_TYPE_INVALIDEFFECT )
{
    ...
    while( GetIsEffectValid( eEffect ) )
    {
        ...
        // Lorinton change
        // Switch from supernatural to extraordinary
...
void SEI_ApplyEffectToObject( int a_nDurationType, effect a_eEffect, object a_oTarget, float a_fDuration = 0.0, int a_bPermanent = FALSE )
{
    ...
    // Lorinton change
    // Switch to extraordinary from supernatural
    AssignCommand( ( a_bPermanent ? GetModule() : GetArea( a_oTarget ) ),
        ApplyEffectToObject( a_nDurationType, ExtraordinaryEffect( a_eEffect ), a_oTarget, a_fDuration ));
    ...
    if( GetIsObjectValid( a_oCharacter ) )
    {
        ...
        if( nSubrace != SUBRACE_NONE )
        {
            ...
            if( stSubrace.m_nStonecunning )
            {
                ...
                if( nGroundSetting == A_UNDERGROUND )
                {
                    // If the character just went underground.
                    if( nCharGround != A_UNDERGROUND )
                    {
                        ...
                        SetLocalInt( a_oCharacter, GROUND_SETTING, A_UNDERGROUND );
                    } // End if
                }
                else
                {
                    // If the character just came above ground.
                    if( nCharGround != A_ABOVEGROUND )
                    {
                        ...
                        SetLocalInt( a_oCharacter, GROUND_SETTING, A_ABOVEGROUND );
                    } // End if
                } // End if-else
            } // End if (Stonecunning)
...
void SEI_ApplySubraceTraits( struct Subrace a_stSubrace, object a_oCharacter )
{
    ...
    // Lorinton change
    // Using a mix of hide and effects to do this as the effects are disappearing at times.
    // The subraces must be defined so that all attributes which can be placed on a hide are.
    // The remaining attributes will be handled here.
    if( !USE_SUBRACE_HIDE || USE_SUBRACE_MIX )
    {
        // Global object where the values are stored.
        object oModule = GetModule();
...
void SEI_InitSubraceTraits( object a_oCharacter, int a_bIncludeItems = TRUE )
{
    ...
    if( nSubrace != SUBRACE_NONE )
    {
//        SendMessageToPC( a_oCharacter, "Loading subrace..." );
        // Load the information on the character's subrace.
        struct Subrace stSubrace = SEI_LoadSubrace( nSubrace );
...
//        SendMessageToPC( a_oCharacter, "Applying subrace..." );
...
void SEI_RemoveSubrace( object a_oCharacter, int a_bRemoveItems = TRUE )
{
    if( GetIsObjectValid( a_oCharacter ) )
    {
        if( a_bRemoveItems )
        {
            ...
            SEI_RemoveSubraceHide( a_oCharacter );
            int nSpellItem;
            for( nSpellItem = 1 ; nSpellItem <= 4 ; ++nSpellItem )
            {
...
void SEI_InitSubrace( object a_oCharacter )
{
    ...
    if( GetIsObjectValid( a_oCharacter ) )
    {
//        SendMessageToPC( a_oCharacter, "Respawning subrace..." );
        // Get the character's subrace.
        int nSubrace = GetLocalInt( a_oCharacter, SUBRACE_FIELD );
        SEI_RemoveSubrace( a_oCharacter, FALSE );
//        SendMessageToPC( a_oCharacter, "Removing subrace..." );
        // See if this character's subrace has been initialized before.
        if( nSubrace == SUBRACE_NONE )
        {
//            SendMessageToPC( a_oCharacter, "Initializing empty subrace..." );
        ...
        {
//            SendMessageToPC( a_oCharacter, "Initializing subrace..." );
            // Initialize the subrace traits on the character.
            SEI_InitSubraceTraits( a_oCharacter );
        } // End if
    } // End if
