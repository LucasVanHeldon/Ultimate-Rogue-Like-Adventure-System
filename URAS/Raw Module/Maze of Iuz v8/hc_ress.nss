...
void main()
{
    ...
    int iRezAmount;
    if (nGold >= (iRezAmount=70*iLevel+500*nBaseCost))
        casttype = RESS;
    if (casttype == NONE)
    {
    ...
    if(GetIsObjectValid(oRespawner))
    {
        ...
        //{
        //        DelayCommand(7.0,ExecuteScript("hc_bleeding", oRespawner));
        //}
    ...
    else
    {
        ...
        if (nPersist)
        {
          ...
          SetCampaignInt("HCRPC" + sID, "PlayerState", 7);
        }
        //SPS(oRespawner,PWS_PLAYER_STATE_RESURRECTED);
