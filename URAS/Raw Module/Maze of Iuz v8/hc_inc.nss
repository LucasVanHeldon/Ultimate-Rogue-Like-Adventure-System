...
int GPS(object oPC)
{
   ...
   if(GetLocalInt(oMod, "PERSIST"))
    return GetCampaignInt("HCRPC" + GetName(oPC)+ GetPCPlayerName(oPC), "PlayerState");
   else
    return GetPersistentInt(oMod, "PlayerState"+ GetName(oPC)+ GetPCPlayerName(oPC));
...
void SPS(object oPC, int nPS)
{
    // hcr3 persist
   if(GetLocalInt(oMod, "PERSIST"))
    SetCampaignInt("HCRPC" + GetName(oPC) + GetPCPlayerName(oPC), "PlayerState", nPS);
   else
    SetPersistentInt(oMod, "PlayerState"+ GetName(oPC) + GetPCPlayerName(oPC), nPS);
