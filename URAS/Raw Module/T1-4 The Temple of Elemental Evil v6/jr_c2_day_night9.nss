...
//:: Updated On: 2003-08-20 - Georg Zoeller - Added check for variables to
//::                                          active spawn in conditions without
//::                                          changing the spawnscript.
//::
//:: Updated On 2005-02-27 - CFX - For HCR 3.03.0b - Added checks to work with
//::                                                 the corrected Racial Movement
//::                                                 & Armor Encumberance toggles
//::////////////////////////////////////////////////////////////////////////////
#include "X0_I0_Anims"
...
void main()
{
  ...
  // * If this is set, the NPC will appear using the "EffectAppear" animation
  // * instead of fading in, *IF* SetListeningPatterns() is called below.
  ...
  // * tag "NW_INTERACTIVE" and "talk" to each other. Humanoid races are
  // * civilized by default, so only set this flag for monster races that you
  // * want to behave the same way.
  ...
  // * Civilized creatures with this flag will randomly use a few voicechats.
  // * It's a good idea to avoid putting this on multiple creatures using the
  // * same voiceset.
  //SetAnimationCondition(NW_ANIM_FLAG_CHATTER);

  // * Creatures with _immobile_ ambient animations can have this flag set to
  // * make them mobile in a close range. They will never leave their immediate
  // * area, but will move around in it, frequently returning to their starting
  // * point. Note that creatures spawned inside interior areas that contain a
  // * waypoint with one of the tags "NW_HOME", "NW_TAVERN", "NW_SHOP" will
  // * automatically have this condition set.
  ...
  // * Ranged attacker.
  // * Will attempt to stay at ranged distance from their target.
  ...
  // * Ambusher.
  // * Will go stealthy/invisible and attack, then run away and try to go
  // * stealthy again before attacking anew.
...
/*
   If you uncomment any of these conditions, the creature will fire a specific
  user-defined event number on each event. That will then allow you to write
  custom code in the "OnUserDefinedEvent" handler script to go on top of the
  default NPC behaviors for that event.

   Example: I want to add some custom behavior to my NPC when they are damaged.
  I uncomment the "NW_FLAG_DAMAGED_EVENT", then create a new user-defined script
...
*/
  ...
  // HCR - Change to allow respawn to happen in same spot without waypoints.
  SetLocalLocation(OBJECT_SELF, "spawn", GetLocation(OBJECT_SELF));

/*
  // HCR - Change to check for Racial Movement toggle
  if (GetLocalInt(oMod, "RACIALMOVEMENT"))
      HC_SetRacialMovementRate(OBJECT_SELF);

  // HCR - Change to check for armor movement encumberance
  if (GetLocalInt(oMod, "ARMORENCUMBER"))
      HC_EffectArmorEncumbrance(OBJECT_SELF);
*/
}
//::////////////////////////////////////////////////////////////////////////////
