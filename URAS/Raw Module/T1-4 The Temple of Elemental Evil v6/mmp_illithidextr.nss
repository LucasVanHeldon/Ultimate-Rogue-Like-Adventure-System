...
void main()
{
    ...
    SendMessageToPC(GetFirstPC(),"Attempting Brain Extract");
    ...
    switch (nEvent)
    {
        case X2_ITEM_EVENT_ONHITCAST:
        {
            ...
            nDC = GetReflexSavingThrow(oSpellTarget);
            nD20 = d20();
            if( (nDC+nD20) < 14)
            {
                SendMessageToPC(GetFirstPC(),"Brain Extract");
                effect eE = EffectKnockdown();
                ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eE, oSpellTarget, 10.0);
                eE = EffectDeath(TRUE);
                DelayCommand(1.0,ApplyEffectToObject(DURATION_TYPE_INSTANT,eE,oSpellTarget));
            }
